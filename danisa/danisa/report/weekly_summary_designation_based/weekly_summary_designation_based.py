...
def get_conds(filters):
	conds = ""
	conds += " AND ompany = %(company)s " if filters.get("company") else ""
	# conds += " AND e.designation = %(designation)s " if filters.get("designation") else ""
	conds += " AND attendance_date between %(from_date)s AND %(to_date)s " if filters.get("from_date") and filters.get("to_date") else ""
	return conds

def get_emp_conds(filters):
	conds = ""
...
def get_columns(date_list):
	columns = [ _("Employee Name") + "::90", _("ID. No.") + "::150",_("Phone No") + "::150"]
	for day in range(len(date_list)):
		columns.append(cstr(day+1) +"::20")
	columns += [_("Amount") + ":Currency:95"]
	return columns
...
def get_results(filters,date_list,conds):
	...
	for emp in sorted(att_map):
		...
		if not emp_det:
			continue
		row = [emp_det.employee_name, emp_det.designation, emp_det.cell_number]
...
def get_employee_details(filters):
	emp_conds = get_emp_conds(filters)
	emp_map = frappe._dict()
	for d in frappe.db.sql("""select name, employee_name, branch, designation,cell_number,company
		from tabEmployee where status = 'Active' %s """  % emp_conds, filters, as_dict=1):
